...
This script analyzes frames of a trajectory that has been split up into many PDB files and produces a contact map between the alpha carbons of chain A and chain B.
...
Useful slices from ATOM lines in pdb files:
    residue number = int((str(line)[23:26]).strip())

Based on script written by Erik Cook.

Created on Thu Apr 23 10:33:36 2020
...
for file in files:
    ...
    for line in pdb_file:
        if 'ATOM' in line and line[21:22] in ['B']:
        ...
        if 'ATOM' in line and line[21:22] in ['A']:       
            appendage = [float((str(line)[30:38]).strip()), float((str(line)[39:46]).strip()), float((str(line)[46:54]).strip())]    
...
colormap = cm.get_cmap(cmap, 256) #the cmap color scheme was reversed by adding '_r'
...
plt.xlabel('rap74 residue number')
plt.ylabel('ctfcp1 residue number')
