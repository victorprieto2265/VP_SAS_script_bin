...
This script plots a heatmap with the output of contact_map_distances.py,
 which analyzes frames of a trajectory that has been split up into many
 PDB files and exports distances between ALL alpha carbons of chain A
 and chain B (both intramolecular and intermolecular contacts).

This script has been specifically adapted for producing a contact map
between alpha carbons of FCP1 and RAP74 to process the trajectories run
by Scott in April/May 2020.

Note that residues 1-67 correspond to RAP74 452-520, 68-71 corresponds
to the GPGW cloning artifact + exogenous tryptophan installed on ctFCP1,
and 72-153 correspond to ctFCP1 879-960.
...
new_path = ('/Users/victorprieto/Desktop/Research/python/'
            + 'trajectory_scripts/trajectory_calc_outputs/'
            + construct
            + '_distance_contact_maps/')
...
"""
this following line fixes an error in the contact map calculator, which
itself was fixed in June 2020.
"""
...
for filename in filenames:
    ...
    """
    point A: line[0] through line[2] becomes residue i, residue j, and
    distance

    point B: this activates when j restarts to 1 and it's not the first
    line. It would be simpler to ask if line[1] == total number of
    residues, but this is written to run regardless of the total number
    of residues.

    point C: appends list of distances described above to array

    point D: necessary to include last distance list in the array, since
    the for loop above doesn't catch the last distance

    """

    for line in distance_file:
        line = line.split()  # point A
        if line[1] == '1' and line[0] != '1':  # point B
            distance_array.append(distance_list)  # point C
            distance_list = []
        distance_list.append(float(line[2]))
        count += 1

    distance_array.append(distance_list)  # point D
