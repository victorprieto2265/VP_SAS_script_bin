...
This script analyzes the output of short_dist_pdb_selector.py files and saves a .txt file of distances per temperature.
...
construct = input('enter the fcp1 construct name (just the code, e.g. 5KR) (wrapped in quotes): ')
...
new_path = '/sas_syn/Data_VP/trajectory_calc_outputs/' + construct + '_short_distances/'
try:
    os.mkdir(new_path)
except:
    None
...
for i in range(0, 20):
    dir_path = '/sas_syn/Data_VP/split_trajectories/' + construct + '_short_distance_files/{:02d}'.format(i) + '_'
#    split_CA_N_' + "{:03d}".format(i) + '_campari_traj/{:02d}'.format(i) + '_'
